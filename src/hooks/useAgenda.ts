...
const STORAGE_KEY = "chongqing-agenda-v1.8";
...
export function useAgenda() {
        ...
        if (saved) {
            try {
                ...
                if (Array.isArray(parsed)) {
                    ...
                    setDays(parsed.days && parsed.days.length > 0 ? parsed.days : generateDefaultItinerary());
                    setUnscheduled(parsed.unscheduled || []);
                }
            } catch (e) {
                console.error("Failed to parse agenda", e);
                setDays(generateDefaultItinerary());
            }
        } else {
            setDays(generateDefaultItinerary());
        }
    ...
    const updateDate = (dayId: string, newDate: string) => {
        setDays((prev) => {
            ...
            for (let i = index; i < newDays.length; i++) {
